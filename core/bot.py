...
class Bot(commands.AutoShardedBot):
    ...
    async def on_connect(self) -> None:
        log.info(f"Successfully connected to discord.")
        if "-sync" in sys.argv:
        ...
        if embed:
            if interaction.response.is_done():
                return await interaction.followup.send(
                    embed=Embed(description=message, colour=discord.Colour.green()),
                    ephemeral=ephemeral
                )
    ...
    async def error(
        ...
        if embed:
            if interaction.response.is_done():
                return await interaction.followup.send(
                    embed=Embed(description=message, colour=discord.Colour.red()),
                    ephemeral=ephemeral
                )
            return await interaction.response.send_message(
